...
async function startImgGen(){
  ...
  const asp=document.getElementById('imgAsp').value,res=model==='nano-banana-2'?'1K':'2K';
  const btn=document.getElementById('imgBtn');btn.disabled=true;btn.innerHTML='<span style="flex:1;text-align:center;background:linear-gradient(90deg,#FFCC00,#FF6600);-webkit-background-clip:text;-webkit-text-fill-color:transparent;font-weight:800;font-size:17px;">Generating…</span><div id="imgBtnBar" style="position:absolute;bottom:0;left:0;height:4px;width:0%;background:linear-gradient(90deg,#FFCC00,#FF6600);border-radius:0 0 12px 12px;box-shadow:0 0 8px rgba(255,160,0,0.6);"></div>';btn.style.position='relative';btn.style.overflow='hidden';btn.style.minHeight='44px';btn.style.padding='10px 20px';var _barPct=0;btn._barTick=setInterval(function(){if(_barPct<90){_barPct+=Math.random()*2;var b=document.getElementById('imgBtnBar');if(b)b.style.width=Math.min(_barPct,90)+'%';}},1000);
